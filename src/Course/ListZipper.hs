...
setFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
setFocus f (ListZipper l a r) = ListZipper l f r
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p (ListZipper l a r) = let (bl, br) = break p l in
  case br of h :. t -> IsZ $ ListZipper t h (foldLeft (flip (:.)) (a :. r) bl)
             Nil    -> IsNotZ
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p (ListZipper l a r) = let (bl, br) = break p r in
  case br of h :. t -> IsZ $ ListZipper (foldLeft (flip (:.)) l (a :. bl)) h t
             Nil    -> IsNotZ
