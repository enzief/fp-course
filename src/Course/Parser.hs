...
character ::
  Parser Char
character = P $ \l -> case l of c :. r -> Result r c
                                Nil    -> UnexpectedEof
...
(|||) ::
  ...
  -> Parser a
  -> Parser a
(P p1) ||| (P p2) = P $ \i -> let res1 = p1 i in if isErrorResult res1 then p2 i else res1
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  f =<< (P p) = P $ \i -> onResult (p i) (\i2 a -> parse (f a) i2)
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser = foldRight (\pa pl -> (\a -> ((:.) a) <$> pl) =<< pa) (pure Nil)
...
surnameParser ::
  Parser Chars
surnameParser = -- (\u -> (\fv -> (\l -> u :. fv ++ l) <$> (list lower)) =<< (thisMany 5 lower)) =<< upper
  do
    u <- upper
    v <- thisMany 5 lower
    l <- list lower
