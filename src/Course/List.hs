...
filter ::
  ...
  -> List a
  -> List a
filter p = foldRight (\x y -> if p x then x :. y else y) Nil
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight (\oa ol -> bindOptional (\a -> mapOptional (a :.) ol) oa) (Full Nil)
...
find ::
  ...
  -> Optional a
find _ Nil      = Empty
find p (a :. t) = if p a then Full a else find p t
