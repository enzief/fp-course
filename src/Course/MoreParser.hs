...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok l r pa = between (charTok l) (charTok r) pa
...
hex ::
  Parser Char
hex = (\ds -> chr $ (readHex ds) ?? (-1)) <$> (thisMany 4 (satisfy isHexDigit))
...
eof ::
  Parser ()
eof = P $ \i -> case i of Nil -> Result Nil ()
                          _   -> ExpectedEof i
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma l r pa = betweenCharTok l r $ sepby pa (is ',')
